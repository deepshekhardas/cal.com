...
import type { ConfirmOptions } from "../../../trpc/server/routers/viewer/bookings/confirm.types";

export class ConfirmBookingService {
    private log = logger.getSubLogger({ prefix: ["ConfirmBookingService"] });

    constructor(private prisma: PrismaClient) { }

    async confirm({ ctx, input }: ConfirmOptions) {
        const {
            bookingId,
            recurringEventId,
        ...
        const bookingAccessService = new BookingAccessService(this.prisma);
        ...
        const organizerOrganizationProfile = await this.prisma.profile.findFirst({
            where: {
                userId: user.id,
            },
        });
        ...
        if (recurringEventId && recurringEvent) {
            const groupedRecurringBookings = await this.prisma.booking.groupBy({
                where: {
        ...
        if (confirmed) {
            ...
            if (recurringEventId) {
                const unconfirmedRecurringBookings = await this.prisma.booking.findMany({
                    where: {
                        recurringEventId,
    ...
    private async fireRejectionEvent({
        actor,
        organizationId,
        actionSource,
        rejectedBookings,
